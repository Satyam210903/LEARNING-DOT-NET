...
namespace VMS
{
    public partial class In : Form
    {
        ...
        private void MakeCheckIn(string mob)
        {
            string currentdate = DateAndTime.Now.ToString();
            try
            {
                string CheckUser1 = "Select Count(*) from visit_pass where mob = @mob and status = 0 ";
                using (SqlCommand Cmd1 = new SqlCommand(CheckUser1, conn))
                {
                    ...
                    {
                        MessageBox.Show("You are already checkedIN.", "Error");
                        conn.Close();
                    }
                }
        ...
        private void StartCamera()
        {
            ...
            // Stop previous camera instance if running
        ...
        private void CheckIN(string mob)
        {
            string currentdate = DateAndTime.Now.ToString("dd/MM/yyyy");
            try
            {
